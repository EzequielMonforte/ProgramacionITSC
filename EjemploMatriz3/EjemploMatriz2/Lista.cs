...
namespace EjemploMatriz2
{
    ...
    public class lista
    {
        ...
        /// </summary>
        /// <returns></returns>
        private System.String[] Redimensionar(int redimensionamiento)
        {
            System.String[] Res = new System.String[CantElem+redimensionamiento];
            System.Int32 j = 0;
            foreach (System.String mElemento in Lista)
            {
				if (j < Res.Length)
        ...
        public System.String BuscarPorPosicion(System.String aPosicion)
        {
			...
			try
			{
				...
				return mRes = "ingrese un valor numerico!";
			}
        ...
        public System.String BorraElemento(System.String aElemento)
        {
			...
			if (mPosicion == -2)
			{
				return mRes = "No se encontro el elemento";
			}
            if (mPosicion == -1)
            {
		...
		/// </summary>
        private void OrdenarAlfa()
        {
			if (Lista.Length < 1)
				return;
			for (int i = 0; i < Lista.Length; i++) {
				for (int o = i+1; o < Lista.Length; o++) {
					if (Lista[i].CompareTo(Lista[o]) > 0) {
						string aux = Lista[i];
						Lista[i] = Lista[o];
						Lista[o] = aux;
